...
from tensorflow.keras.layers import LSTM, Dense #LSTM is a type of recurrent neural network layer suitable for handling sequential data. Dense is a fully connected layer used for outputting final prediction
...
def create_lstm_model(x_train, y_train, look_back = 1):
    ...
    '''
    model.add(LSTM(50, activation='relu', input_shape=(look_back, x_train.shape[2]))) #Adds an LSTM later with 50 unites(memory cells)
    ...
    model.compile(optimizer='adam', loss='mse') #configures the learning process. optimzier = 'adam' uses the adam optimization algorithm. loss='mse' uses the mean squared error loss function, whihc is common for regression problems
    model.fit(x_train, y_train, epochs=100, verbose=0)#trains the model using the training data. epoch =100 the number of times the entire training dataset is passed through the model. verbose = 0 suppresses the training progress output

    return model #we retunr the trained model
...
def prepare_lstm_data(data, look_back = 1):
    ...
    for i in range(len(data) - (look_back-1)): #creates sequences of length(look_back)
...
def train_lstm_model(prices):
    ...
    train_size = int(len(x) * 0.9) #calculating the size of the training set(80% of the data)
